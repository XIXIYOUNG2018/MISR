...
# f=open('clf_weight.pkl','ab')
# g=open('pred.pkl','ab')
# file=open('smiles.pkl','ab')
# f1=open('y.pkl','ab')
# f2=open('node_seq.pkl','ab')
class Graphormer(pl.LightningModule):
    ...
    def forward(self, batched_data, perturb=None):
        ...
        #print(torch.max(rel_pos))
        zeros=torch.zeros_like(rel_pos)
        rel_pos=torch.where(rel_pos>64,zeros,rel_pos)
        rel_pos_bias = self.rel_pos_encoder(rel_pos)
        #print(torch.max(rel_pos))
        ...
        # pickle.dump(batched_data.x.to('cpu').numpy(),f2)
        # pickle.dump(batched_data.smiles,file)
        # pickle.dump(batched_data.y.to('cpu').numpy(),f1)

        reg_output,weight=self.clf_head(batched_data,output)  #fine tune with clf task
        #print(reg_output)
        # print(type(weight),len(weight),len(weight[0]))
        # weight=weight[6]
        #weight=weight[np.arange(len(batched_data)),batched_data.task_num]
        # weight=weight.to('cpu').numpy()
        # pickle.dump(weight,f)
        # pickle.dump(batched_data.smiles,file)
        # pickle.dump(batched_data.y.to('cpu').numpy(),f1)
        reg_output=reg_output[np.arange(len(batched_data)),batched_data.task_num]
        # pickle.dump(reg_output.to('cpu').numpy(),g)  
        return reg_output


    def training_step(self, batched_data, batch_idx):

        pred_c=self(batched_data)  #fine tune with reg task
        #pred_r=pred_r[np.arange(len(batched_data)),batched_data.task_idx]
        y_c=batched_data.y.float()
        #print(pred_c,y_c,pred_c.shape,y_c.shape)
        # print(len(y_c))
        loss=nn.functional.binary_cross_entropy_with_logits(pred_c,y_c,reduction='mean',pos_weight=torch.tensor([4]).to('cuda:1'))
    ...
    def validation_epoch_end(self, outputs):
        y_pred_c=torch.cat([i['y_pred_c'] for i in outputs])
        y_true_c=torch.cat([i['y_true_c'] for i in outputs])
        input_dict = {
    ...
    def test_step(self, batched_data, batch_idx):
        pred_c=self(batched_data)  #fine tune with clf task
        y_c=batched_data.y.int() 
        return {
            'y_pred_c': pred_c,
            'y_true_c': y_c,
    ...
    def test_epoch_end(self, outputs):
        ...
        y_pred_c=torch.cat([i['y_pred_c'] for i in outputs])
        y_true_c=torch.cat([i['y_true_c'] for i in outputs])
        input_dict = {"y_true_c":y_true_c,"y_pred_c":y_pred_c}
